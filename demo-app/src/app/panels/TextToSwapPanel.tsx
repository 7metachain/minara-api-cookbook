"use client";

import { useState } from "react";
import CodeBlock from "../../components/CodeBlock";
import ResponseBlock from "../../components/ResponseBlock";

const CHAINS = ["base", "ethereum", "arbitrum", "optimism", "bsc"];

export default function TextToSwapPanel() {
  const [intent, setIntent] = useState("swap 0.1 ETH to USDC");
  const [walletAddress, setWalletAddress] = useState(
    "0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb1"
  );
  const [chain, setChain] = useState("base");
  const [data, setData] = useState<unknown>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  const handleSubmit = async () => {
    setLoading(true);
    setError(null);
    setData(null);
    try {
      const res = await fetch("/api/text-to-swap", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ intent, walletAddress, chain }),
      });
      const json = await res.json();
      if (!res.ok) throw new Error(json.error || `HTTP ${res.status}`);
      setData(json);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Unknown error");
    } finally {
      setLoading(false);
    }
  };

  const codeSnippet = `const response = await fetch(
  "https://api-developer.minara.ai/v1/developer/intent-to-swap-tx",
  {
    method: "POST",
    headers: {
      Authorization: "Bearer YOUR_API_KEY",
      "Content-Type": "application/json",
    },
    body: JSON.stringify({
      intent: ${JSON.stringify(intent)},
      walletAddress: "${walletAddress}",
      chain: "${chain}",
    }),
  }
);

const data = await response.json();
// data.intent      → parsed swap details
// data.quote       → price impact, gas, fees
// data.unsignedTx  → ready-to-sign transaction
// data.approval    → ERC20 approval status`;

  return (
    <div className="grid gap-6 lg:grid-cols-2">
      <div className="space-y-4">
        <div className="rounded-lg border border-card-border bg-card p-4">
          <h3 className="mb-4 text-sm font-semibold">
            POST /v1/developer/intent-to-swap-tx
          </h3>

          <label className="mb-1 block text-xs text-muted">
            Swap Intent (natural language)
          </label>
          <input
            type="text"
            value={intent}
            onChange={(e) => setIntent(e.target.value)}
            className="mb-4 w-full rounded-md border border-card-border bg-background px-3 py-2 text-sm outline-none focus:border-accent"
            placeholder='e.g. "swap 100 USDC to SOL"'
          />

          <label className="mb-1 block text-xs text-muted">
            Wallet Address
          </label>
          <input
            type="text"
            value={walletAddress}
            onChange={(e) => setWalletAddress(e.target.value)}
            className="mb-4 w-full rounded-md border border-card-border bg-background px-3 py-2 font-mono text-xs outline-none focus:border-accent"
            placeholder="0x..."
          />

          <label className="mb-1 block text-xs text-muted">Chain</label>
          <div className="mb-4 flex flex-wrap gap-2">
            {CHAINS.map((c) => (
              <button
                key={c}
                onClick={() => setChain(c)}
                className={`rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${
                  chain === c
                    ? "bg-accent text-white"
                    : "bg-background text-muted hover:text-foreground border border-card-border"
                }`}
              >
                {c}
              </button>
            ))}
          </div>

          <button
            onClick={handleSubmit}
            disabled={loading || !intent.trim()}
            className="w-full rounded-md bg-accent px-4 py-2.5 text-sm font-medium text-white transition-colors hover:bg-accent-light disabled:opacity-50"
          >
            {loading ? "Generating..." : "Generate Swap Transaction"}
          </button>
        </div>

        <ResponseBlock data={data} loading={loading} error={error} />
      </div>

      <div>
        <CodeBlock
          title="Code Snippet — copy this into your project"
          code={codeSnippet}
        />
      </div>
    </div>
  );
}
